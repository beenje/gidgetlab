...
class TestEvent:
    ...
    def test_init(self):
        ins = sansio.Event(self.data, event=self.headers["x-gitlab-event"],
                           secret=self.secret)
    ...
    def test_from_http_missing_token(self):
        ...
        with pytest.raises(ValidationFailure):
            sansio.Event.from_http(headers_no_token, self.data_bytes,
                                   secret=self.secret)
...
class TestRateLimit:
    ...
    def test_from_http(self):
        ...
        rate = 65
        reset = datetime.datetime.now(datetime.timezone.utc)
        headers = {"ratelimit-limit": str(rate),
...
class TestDecipherResponse:
    ...
    def test_next(self):
        ...
        data, rate_limit, more = sansio.decipher_response(status_code, headers,
                                                          body)
        assert more == "https://gitlab.com/api/v4/groups/gitlab-org/projects?archived=false&id=gitlab-org&order_by=created_at&owned=false&page=2&per_page=20&simple=false&sort=desc&starred=false&with_custom_attributes=false&with_issues_enabled=false&with_merge_requests_enabled=false"
        ...
        data, rate_limit, more = sansio.decipher_response(status_code, headers,
                                                          body)
        assert more == "https://gitlab.com/api/v4/groups/gitlab-org/projects?archived=false&id=gitlab-org&order_by=created_at&owned=false&page=3&per_page=20&simple=false&sort=desc&starred=false&with_custom_attributes=false&with_issues_enabled=false&with_merge_requests_enabled=false"
        assert rate_limit.remaining == 598
